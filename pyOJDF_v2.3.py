...
def newstep(T,F,YF,YO,U,W):
	...
	TN=np.zeros(25)
        ...
        TN[0]=dumT[0]+(((dumT[1]-dumT[0])/(dy*PR))+XL*dumF[0])*dts
	FN[0]=-beta*exp(-EW/dumT[0])
        ...
        for Fi, I in zip(dumF[1:M1], range(1,M1)):
        	...
        	if Fi >= 0:
        		...
        		UL =dumU[I]
        		UR =dumU[I+1]
        	TN[I] =dumT[I] +A*db*((dumT[I+1]-2.*dumT[I]+dumT[I-1])/PR)+A*da*dumF[I]*(TR-TL)+dt*Q*dumW[I]
        	YFN[I]=(dumYF[I]+A*db*(dumYF[I+1]-2.*dumYF[I]+dumYF[I-1])/SC +A*da*dumF[I]*(YFR-YFL))/(1.0+ dt*D*dumYO[I]*ARH)
        	YON[I]=(dumYO[I]+A*db*(dumYO[I+1]-2.*dumYO[I]+dumYO[I-1])/SC +A*da*dumF[I]*(YOR-YOL))/(1.0-dt*XNO*D*dumYF[I]*ARH)
        	UN[I] =dumU[I] +A*da*dumF[I]*(UR-UL) +A*db* (dumU[I+1]-2.*dumU[I]+dumU[I-1])+A*dt*(dumT[I]-dumU[I]*dumU[I])
        
	TN[0]=-EW/log(-1.0*FN[0]/beta)
	UN[0]=0.
	ACT1=E/TN[0]
	ARH1=1.0/exp(ACT1)
	YF0J=dumF[0]*2.0*dy*(dumYF[0]-1.0)*SC +dumYF[1]
        YFN[0]=(dumYF[0]+A*db*(dumYF[1]-2.*dumYF[0]+YF0J)/SC+A*.5*da*(dumYF[1]-YF0J)*dumF[0]) /(1.0 +A*dt*D*dumYO[0]*ARH1)
        YO0J=dumF[0]*2.0*dy*dumYO[0]*SC +dumYO[1]
        YON[0]=(dumYO[0]+A*db*(dumYO[1]-2.*dumYO[0]+YO0J)/SC+.5*A*da*(dumYO[1]-YO0J)*dumF[0])/(1.0-XNO*A*dt*D*dumYF[0]*ARH1)
